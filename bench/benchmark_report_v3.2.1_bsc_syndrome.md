...
The Inversion Index (II) quantifies the fraction of trials that are syndrome-consistent but logically incorrect — isolating the "wrong but structurally valid" decoding regime:
...
Properties:
- **II > 0** indicates the presence of syndrome-consistent but logically incorrect corrections (inversion regime).
- **II = 0** indicates that all syndrome-consistent corrections are also logically correct, or that neither syndrome consistency nor logical correctness is achieved.
- **II peaks under oracle channel at p > 0.50** (Regime III), where the decoder converges to syndrome-consistent but sign-inverted (logically wrong) corrections.
- **II ≈ 0 under syndrome-only channel**, because the uniform LLR provides no directional bias to steer the decoder toward syndrome-consistent wrong answers.

The Inversion Index is an exact algebraic derivative of existing fields (FER, SCR). No new data sources or benchmark runs are required.
...
## 7. Inversion Index Tables (Below-Threshold Region)

The Inversion Index (II = SCR - Fidelity) isolates syndrome-consistent but logically incorrect decoding outcomes.
...
### Near-Critical Region (p_near) — Inversion Index Summary

**II ≈ 0.000** across all (schedule, distance, p) combinations in the near-critical region (p >= 0.45). Since Fidelity = 0.0 everywhere, II equals SCR, which is itself near zero. No systematic inversion regime exists under the syndrome-only channel.

### Combined Sample Table — Inversion Index Derivation

| p | schedule | distance | FER | Fidelity | SCR | Inversion Index |
|---|----------|----------|-----|----------|-----|-----------------|
| 0.01 | flooding | d=3 | 0.265000 | 0.735000 | 0.740000 | 0.005000 |
| 0.02 | layered | d=3 | 0.470000 | 0.530000 | 0.550000 | 0.020000 |
| 0.1 | hybrid_residual | d=3 | 0.975000 | 0.025000 | 0.055000 | 0.030000 |
| 0.02 | adaptive | d=5 | 0.655000 | 0.345000 | 0.370000 | 0.025000 |
| 0.05 | hybrid_residual | d=5 | 0.945000 | 0.055000 | 0.075000 | 0.020000 |
| 0.02 | flooding | d=7 | 0.805000 | 0.195000 | 0.215000 | 0.020000 |
| 0.49 | flooding | d=5 | 1.000000 | 0.000000 | 0.000000 | 0.000000 |

All Inversion Index values are verified as exact: II = SCR - Fidelity = SCR - (1 - FER).
...
The Inversion Index measures the gap between syndrome consistency and logical correctness. A trial that is syndrome-consistent (the decoder's correction satisfies `H*c = s`) but logically incorrect (the correction differs from the true error by more than a stabilizer) contributes positively to II. By definition:

- Trials that are both syndrome-consistent AND logically correct contribute equally to SCR and Fidelity, canceling in the difference.
- Trials that are syndrome-consistent BUT logically incorrect contribute to SCR but not Fidelity, producing II > 0.
- Trials that fail syndrome consistency contribute to neither, leaving II unchanged.

### Why Inversion Index Peaks Under Oracle Inversion Regime

Under the oracle channel at p > 0.50 (Regime III), the inverted LLR signs cause the decoder to converge in a single iteration to the bitwise complement of the correct error vector. This complemented correction satisfies the syndrome constraints (SCR = 1.0) but is logically incorrect (FER = 1.0, Fidelity = 0.0). The result is II = 1.0 - 0.0 = **1.0**, the maximum possible value.

This is a structural artifact of the oracle channel's position-specific LLR sign inversion, not an intrinsic property of BP decoding.

### Why Inversion Index ≈ 0 Under Syndrome-Only Channel

Under the BSC syndrome-only channel, the uniform LLR `log((1-p)/p)` provides no position-specific information. When the decoder fails (FER > 0), it typically fails to find any syndrome-consistent correction — the output is structurally invalid, not "wrong but valid." The small non-zero II values observed (0.005–0.030) represent rare stochastic events where the decoder found a syndrome-consistent correction that happened to differ from the true error by a non-stabilizer element. These values are consistent with statistical noise at the trial counts used (200–500 trials).

### Regime Summary with Inversion Index

| Regime | p Range | FER | Fidelity | SCR | II | Character |
|--------|---------|-----|----------|-----|----|-----------|
| A — Partial Recovery | p <= ~0.02 | 0.26–0.84 | 0.16–0.74 | ~Fidelity | ~0.00–0.03 | Genuine syndrome-based decoding |
| B — Transition | ~0.02 < p < ~0.10 | 0.80–1.00 | 0.00–0.20 | ~Fidelity | ~0.00–0.02 | Degrading capability |
| C — Complete Failure | p >= ~0.10 | 1.000 | 0.000 | ~0.000 | ~0.000 | No decoding capability |

The near-zero Inversion Index across all regimes confirms the absence of a systematic inversion mechanism under the syndrome-only channel.

---

## 9. Mean Iteration Tables (Below-Threshold Region)
...
## 10. Schedule Differentiation Summary
...
## 11. Regime Interpretation — Syndrome-Only Channel
...
| Inversion Index | 0.000–0.030 (stochastic noise) |
...
| SCR | 0.000–0.215 |
| Inversion Index | 0.000–0.020 (stochastic noise) |
...
| Inversion Index | 0.000–0.015 (equals SCR) |
| mean_iters | 12 (adaptive) / 50 (others) |

**No inversion regime.** Unlike the oracle channel (where p > 0.50 produces SCR = 1.0 with FER = 1.0 and II = 1.0), the syndrome-only channel shows no recovery above any tested p value. The Inversion Index confirms that syndrome-consistent incorrect outcomes do not occur systematically — the near-zero II values are stochastic noise, not evidence of an inversion mechanism.
...
The Inversion Index is computed as an exact algebraic derivative of existing deterministic fields (FER, SCR). It introduces no new stochastic operations and inherits the determinism guarantees of its source metrics.
...
4. **No inversion regime.** The oracle channel's sign inversion at p > 0.50 (producing SCR=1, FER=1, II=1.0) has no analogue under syndrome-only. The uniform LLR magnitude simply approaches zero as p approaches 0.50, and there is no mechanism to produce "wrong but syndrome-consistent" corrections. The Inversion Index confirms this: II ≈ 0.00 across all tested operating points.
...
| Item | Status |
|------|--------|
| Files changed | `bench/benchmark_report_v3.2.1_bsc_syndrome.md` (this file) |
| New metric | Inversion Index (II = SCR - Fidelity) |
| Inversion Index tables | Sections 7 (per-distance tables + combined sample) |
